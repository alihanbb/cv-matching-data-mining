...
from src.pipeline.matching_inputs import MatchingMatrices, build_matching_matrices, rankings_from_fused
...
def _rank_tfidf_baseline(m: MatchingMatrices, top_k: int) -> pd.DataFrame:
    ...
    fused, _ = fuse_scores(m.sim_lex, None, m.skill_score, m.exp_mat, w, False, bm25=None)
    return rankings_from_fused(fused, m.cv_ids, m.job_ids, top_k)


def _rank_tfidf_sbert(m: MatchingMatrices, top_k: int) -> pd.DataFrame:
    w = {"tfidf": 0.5, "dense": 0.5, "skills": 0.0, "experience": 0.0}
    fused, _ = fuse_scores(m.sim_lex, m.dense_sim, m.skill_score, m.exp_mat, w, m.dense_enabled, bm25=None)
    return rankings_from_fused(fused, m.cv_ids, m.job_ids, top_k)
...
def _rank_hybrid_v2(m: MatchingMatrices, top_k: int, cfg: dict[str, Any]) -> pd.DataFrame:
    ...
    fused, _ = fuse_scores(m.sim_lex, m.dense_sim, m.skill_score, m.exp_mat, w, m.dense_enabled, bm25=m.bm25)
    return rankings_from_fused(fused, m.cv_ids, m.job_ids, top_k)
...
def _rank_optimized(m: MatchingMatrices, top_k: int, root: Path) -> pd.DataFrame:
    ...
    if m.bm25 is not None and m.bm25_enabled and bm25_w > 0:
        ...
        fused, _ = fuse_scores(
            m.sim_lex, m.dense_sim, m.skill_score, m.exp_mat, w_full, m.dense_enabled, bm25=m.bm25
        )
    else:
        fused, _ = fuse_scores(m.sim_lex, m.dense_sim, m.skill_score, m.exp_mat, w, m.dense_enabled, bm25=None)
    return rankings_from_fused(fused, m.cv_ids, m.job_ids, top_k)
    ...
    *,
    ...
    if not gt_path.is_file():
        raise FileNotFoundError(
            ...
            "Add data/evaluation/ground_truth.csv (job_id,cv_id,relevance) to run evaluation exports."
        )
    ...
    spec: list[tuple[str, pd.DataFrame]] = [
        ...
        ("TF-IDF + SBERT", _rank_tfidf_sbert(m_sem, top_k)),
